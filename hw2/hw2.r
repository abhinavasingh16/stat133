...
OW_NHANES2 <- (OW_limit == 26 & family$bmi > 26) | (OW_limit == 25 & family$bmi > 26)
...
# weight = (2.2) * (bmi * (2.54/100 * height)^2)
...
abline(plot(family$weight,OW_weight))
...
x <- quantile(BerkArea$price,c(.99))
y <- quantile(BerkArea$bsqft,c(.99),na.rm=TRUE)
BerkArea <- BerkArea[BerkArea$price < x,]
BerkArea <- BerkArea[BerkArea$bsqft < y,]
...
brCols[brCols == 2] <- rCols[2]
brCols[brCols == 3] <- rCols[3]
brCols[brCols == 4] <- rCols[4]
brCols[brCols == 5] <- rCols[5]
